...
import fun.scop.app.isr.Logger._
import fun.scop.csp._
import fun.scop.encode._
import fun.scop.sat._

case class IsrSolverTs01(p: IsrInstance, satsolver: SatSolver)
  ...
  def createCSP(solver: CspSolver, k: Int) = {
    ...
    /** (2-3) トークンスライドの条件 あるトークンが頂点 v から消える ==> 隣接頂点のどれかにトークンが移る
      */
    for (s <- steps.dropRight(1); v <- V) {
      // println(V.toSeq.sorted)
      val adjacentOfV = p.graph.adjacentOf(v)

      val antecedent = to0map(v, s) === 1
      val consequent = Or(adjacentOfV.map(u => to1map(u, s) === 1))

      solver.addCtr(antecedent ==> consequent)
    }
